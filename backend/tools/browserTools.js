    ...
    if (await detectCaptcha(page)) {
      return {
        content: [
          {
            type: "text",
            text: "⚠️ CAPTCHA Detected! Please solve it in the browser window.",
          },
        ],
      };
    }
    ...
    if (await detectCaptcha(page)) {
      return {
        content: [
          {
            type: "text",
            text: "⚠️ CAPTCHA Detected! Solve it in the browser window first.",
          },
        ],
      };
    }
  ...
  try {
    ...
    while (attempts < 10) {
      ...
      const hasResults = await page.evaluate(
        () => document.querySelectorAll("div.g").length > 0
      );
    ...
    const hasResultsNow = await page.evaluate(
      () => document.querySelectorAll("div.g").length > 0
    );
    if (!hasResultsNow) {
      await scrollPage(page);
    ...
    const results = await page.evaluate(() => {
      const items = Array.from(
        document.querySelectorAll("div.g, .v7W49e > div, .MjjYud")
      );
      return items
        .map((item) => {
        ...
        .filter(
          (item) =>
            ...
            !item.link.includes("google.com/search")
        );
    });
    ...
    if (results.length === 0) {
      return {
        content: [
          {
            type: "text",
            text: `No search results found on page "${pageTitle}". Google might be blocking or the page structure changed.`,
          },
        ],
      };
    }
    ...
    return {
      content: [
        { type: "text", text: `❌ Browser search failed: ${error.message}` },
      ],
    };
  }
}
  ...
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully clicked "${selector}". Current URL: ${page.url()}`,
        },
      ],
    };
  } catch (error) {
...
export async function browser_type(selector, text) {
  ...
  try {
    ...
    return {
      content: [
        { type: "text", text: `Successfully typed into "${selector}"` },
      ],
    };
  } catch (error) {
    return {
  ...
  try {
    ...
    return {
      content: [
        {
          type: "text",
          text: `Successfully pressed key "${key}". Current URL: ${page.url()}`,
        },
      ],
    };
  } catch (error) {
    return {
      content: [
        { type: "text", text: `❌ Key press failed: ${error.message}` },
      ],
    };
  }
}
...
export async function browser_wait_for(selector, timeout = 30000) {
  ...
  try {
    ...
    return {
      content: [
        { type: "text", text: `Selector "${selector}" is now visible.` },
      ],
    };
  } catch (error) {
    return {
      content: [
        { type: "text", text: `❌ Wait for selector failed: ${error.message}` },
      ],
    };
  }
}