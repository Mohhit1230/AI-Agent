...
export async function git_check_status() {
  try {
    ...
    if (stderr && !stdout) {
      return { content: [{ type: "text", text: `❌ Git status error: ${stderr}` }] };
    }
    return { content: [{ type: "text", text: `Git Status:\n\n${stdout}` }] };
  } catch (error) {
    return { content: [{ type: "text", text: `❌ Git status failed: ${error.message}` }] };
  }
}
...
export async function github_create_issue(owner, repo, title, body) {
  ...
  if (!token) {
    return { content: [{ type: "text", text: "⚠️ GITHUB_TOKEN is not set in the .env file. Please add it to use GitHub tools." }] };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { content: [{ type: "text", text: `❌ Failed to create GitHub issue: ${errorData.message}` }] };
    }

    const data = await response.json();
    return { content: [{ type: "text", text: `✅ GitHub issue created successfully: ${data.html_url}` }] };
  } catch (error) {
...
export async function git_list_commits(count = 10) {
  try {
    ...
    return { content: [{ type: "text", text: `Latest ${count} commits:\n\n${stdout}` }] };
  } catch (error) {
    return { content: [{ type: "text", text: `❌ Failed to list commits: ${error.message}` }] };
  }
}
export async function git_commit_all(message) {
  try {
    ...
    return { content: [{ type: "text", text: `✅ Committed changes: ${stdout}` }] };
  } catch (error) {
    return { content: [{ type: "text", text: `❌ Git commit failed: ${error.message}` }] };
  }
}
...
export async function git_push() {
  try {
    ...
    return { content: [{ type: "text", text: `✅ Pushed changes: ${stdout}` }] };
  } catch (error) {
    return { content: [{ type: "text", text: `❌ Git push failed: ${error.message}` }] };
  }
}

export async function github_create_pull_request(owner, repo, title, body, head, base = "main") {
  const token = process.env.GITHUB_TOKEN;
  if (!token) return { content: [{ type: "text", text: "⚠️ GITHUB_TOKEN missing." }] };
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) return { content: [{ type: "text", text: `❌ PR failed: ${data.message}` }] };
...
export async function github_list_issues(owner, repo, state = "open") {
  ...
  try {
    ...
    const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues?state=${state}`, { headers });
    const data = await response.json();
    if (!response.ok) return { content: [{ type: "text", text: `❌ Failed: ${data.message}` }] };

    const issues = data.filter(i => !i.pull_request).map(i => `#${i.number} ${i.title} (${i.state})`).join("\n");
    return { content: [{ type: "text", text: `Issues in ${owner}/${repo}:\n\n${issues || "No issues found."}` }] };
  } catch (error) {
    return { content: [{ type: "text", text: `❌ Error: ${error.message}` }] };
  }
}
...
export async function github_list_pull_requests(owner, repo, state = "open") {
  ...
  try {
    ...
    const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls?state=${state}`, { headers });
    const data = await response.json();
    if (!response.ok) return { content: [{ type: "text", text: `❌ Failed: ${data.message}` }] };

    const prs = data.map(p => `#${p.number} ${p.title} (${p.state})`).join("\n");
